...
from app.schemas.ipvSchema import IPAddressDetail, IPAddressRead
...
def create_ip_address(*, db: Session = Depends(get_db), ip_address_in: IPAddressCreate):
...
@router.get("/{ip_address_id}", response_model=IPAddressDetail)
...
@router.put("/{ip_address_id}", response_model=IPAddressRead)
